...
while True:
    ...
    cur.execute("select count(*) from abemavideo where done = 0 and prg_freeendat > %s and prg_freeendat is not null;",(starttime,))
    cnt = cur.fetchone()[0];
    if cnt == 0:
        break
    for genre in ('reality','animation','drama','variety','documentary','movie', 'music'):
        ...
        cur.execute("select  `prg_id` , `prg_title` , `series_title` , `season_title` , `prg_freeendat`  , `prg_url`  from abemavideo where `done`=0 and genre = %s and prg_freeendat > %s and prg_freeendat is not null order by prg_freeendat asc limit 1 ;",(genre,starttime,));
